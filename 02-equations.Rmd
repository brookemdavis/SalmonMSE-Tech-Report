...
\begin{align}
  \textrm{Smolt}^\textrm{NOS}_{y+1} &= \frac{\alpha \times \textrm{Fry}^\textrm{NOS}_{y+1}}{1 + \beta(\textrm{Fry}^\textrm{NOS}_{y+1}   +  \textrm{Fry}^\textrm{HOS}_{y+1} + n^\textrm{sub}_{y+1})}\\
...
where \\ 
  \alpha &= \kappa/\phi\\ 
  \phi &= \sum_a\left(\prod_{i=1}^{a-1}\exp(-M^\textrm{NOS}_i)(1-r_i)\right)\times r_a \times p^\textrm{female} \\ \times \textrm{Fec}_a \\
  \beta &= \alpha/{C_\textrm{egg-smolt}}
(\#eq:EggSmoltBH)
\end{align}

where $\phi$ is the unfished egg per smolt, and $r_a$ as the maturity at age

Smolt production can be predicted from total adult spawners by setting $\textrm{Fec}_a = 1$ and $\phi = 1$.

The density-independent component of the survival equation is controlled by $\alpha$ and the density-dependent component of survival is controlled by $\beta$ and scaled by the total number of fry in competition with subyearling hatchery releases (see Hatchery Production Section). <!--[Hatchery](#hatchery-production) section). not sure about this link format? -->
...
With the Ricker stock-recruit relationship, smolt production is modelled with productivity parameter \textrm{alpha} (maximum recruitment, $\kappa$, scaled by unfished per capita egg production, $\phi$) and capacity parameter \textrm{beta} (inverse of egg production that maximizes smolt production, $\textrm{S}_\textrm{max}$):
<!-- tried to make this more readable but I might have messed it up, make sure phi is what I said it is?-->
...
\widetilde{\textrm{Egg}}^\textrm{NOS}_y =
 ...
 \end{cases}
where \\
C^\textrm{inc*}_y = C^\textrm{inc} \times \textrm{Egg}^\textrm{NOS}_y/(\textrm{Egg}^\textrm{NOS}_y + \textrm{Egg}^\textrm{HOS}_y)
(\#eq:HockeyStick)
\end{align}

where $C^\textrm{inc*}_y$ is the capacity apportioned to natural spawners based on relative abundance.
...
The annual target egg production for the hatchery is calculated from the target releases a:
...
\begin{align}
\textrm{Smolt}^\textrm{Rel}_{y+1} = n^\textrm{yearling}_{y+1} +  \frac{\alpha \times n^\textrm{subyearling}_{y+1}}{1 + \beta(\textrm{Fry}^\textrm{NOS}_{y+1} + \textrm{Fry}^\textrm{HOS}_{y+1} + n^\textrm{subyearling}_{y+1})} (\#eq:SmoltRel1) \\
\end{align}

or  

\begin{align}
\textrm{Smolt}^\textrm{Rel}_{y+1} = n^\textrm{yearling}_{y+1}  + \alpha \times n^\textrm{subyearling}_{y+1}  \times \exp(-\beta(\textrm{Fry}^\textrm{NOS}_{y+1} + \textrm{Fry}^\textrm{HOS}_{y+1} + n^\textrm{subyearling}_{y+1})) (\#eq:SmoltRel2) \\
\end{align}
...
If management is by target catch $K^\textrm{PT,target}$, then the fishing mortality is solved such that

  \begin{align}
  \textrm{K}^\textrm{PT,target} = \sum_a K^\textrm{NOS,PT}_{y,a} + \sum_a K^\textrm{HOS,PT}_{y,a}$
  (\#eq:FishMortCTarget)
\end{align}
